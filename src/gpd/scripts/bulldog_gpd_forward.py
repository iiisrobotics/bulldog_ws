...
def grasp_filtering(grasps, frame='odom',
                       yaw_thresh=1.0472,
                       min_y_thresh=-0.05,
                       min_y_soft_thresh=0.08,
                       min_dist_thresh=0.81):
    """Filter grasps by their positions and orientations in a specified frame.
    ...
    - yaw_thresh: float (optional, default = 1.0472)
        Yaw angle threshold in radian, i.e. the left arm cannot
        move to the right side of the robot, and the right arm cannot
        move to the left side of the robot.
    ...
    #
    # grasp position and orientation filtering
    #
    pose_plannar_positions = [[pose_position.point.x,
                               pose_position.point.y]
                              for pose_position in pose_positions]
    pose_eulers = [tf.transformations.euler_from_matrix(
        np.array([
    ...
    # The yaw angle (top view, positive - counterclockwise,
    # negative - clockwise) should line in the constraint.
    yaws = np.array([pose_euler[2] for pose_euler in pose_eulers])
    orientation_inliner = yaws <= yaw_thresh

    # Only choose the closest grasp.
    dist = np.linalg.norm(pose_plannar_positions, axis=1)
    closest_dist = np.min(dist)
    rospy.loginfo("Closest Distance: %f." % closest_dist)
    position_inliner = dist == closest_dist

    inliner = np.logical_and(orientation_inliner, position_inliner)
    inliner_indices = np.where(inliner)[0]
    closest_grasp = [grasps[inliner_idx] for inliner_idx in inliner_indices]
...
def main():
    ...
    #
    grasp = grasp_filtering(grasp_configs.grasps, GRASP_FILTERING_FRAME)
    ...
    target_euler = tf.transformations.euler_from_matrix(target_rotation_matrix)
    print("Target Euler: ")
    print(target_euler)
